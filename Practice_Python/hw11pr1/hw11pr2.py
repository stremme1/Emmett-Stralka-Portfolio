...
# Below are some boards that will be re-created each time the file is run:
    ...
    def colsToWin(self, ox):
        """return the list of columns where ox can move 
        in the next turn in order to win and finish the game"""
        winnercols=[]
        for i in range(self.col):
            if self.allowsmove(self,ox)==True:
                self.addmove(self,i,ox)
                if winsFor(self,ox)==True:
                    winnercols += [i]
                    self.delMove(self,i)
                    continue
            else: return
    ...
    def aiMove(self, ox):
        """eturn a single integer, 
        which must be a legal column in which to make a move
        If there is a way for ox to win,
        """
        colp=0
        if ox=='O':
            opps= 'X'
        ...
        while True:
            while self.selfallowsMove(self.col)== False:
                if self.selfcolsToWin(self,opps) != []:
                    colp=self.colsToWin(self,opps)
                    self.addmove(self,opps)
                if self.selfcolsToWin(self,ox) != []:
                    for col in [3,4,2,5,1,6,0]:
                        if self.allowsMove(col):
                            return col
                    self.addMove(self,col,ox)
                else:
                    colp=self.colsToWin(self,colp,opps)
                    self.addMove(self,colp,ox)
                    break
